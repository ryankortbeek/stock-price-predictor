...
def init_neural_network(dims):
    ...
    '''
    if len(dims) < 3:
        print('For the sake of this application, neural network must have at least three layers (one input, one hidden, one output).', file=sys.stderr)
        sys.exit(1)
...
def calc_loss_and_risk(Ys, t, loss_as_vec=False):
    ...
    # Mean square error
    loss = (1 / len(Ys[-1])) * (Ys[-1] - t) ** 2 if loss_as_vec else (1 / len(Ys[-1])) * np.linalg.norm(Ys[-1] - t, 2) ** 2
    # Mean absolute difference
    risk = (1 / len(Ys[-1])) * np.linalg.norm(np.absolute(Ys[-1] - t), 1)
    return loss, risk
...
def do_backward_propagation(w, Ys, t):
    ...
    for i in range(len(w) - 1, 0, -1):
        dws.append(np.dot(dws[-1], np.transpose(w[i])) * sigmoid_derivative_v(Ys[i]))
    return dws, loss
...
def mlp_train(X_train, t_train, X_val, t_val, hyperparams):
    ...
    for epoch in range(hyperparams.max_epochs):
        ...
        for b in range(num_batches):
            ...
            for i in range(len(w)):
                w[i] -= hyperparams.alpha * np.dot(np.transpose(Ys[i]), dws[-(i + 1)])
